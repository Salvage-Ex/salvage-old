...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), mainParams.GetDefaultPort(), testNetParams.GetDefaultPort()));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, salvage, (Darksend, Instantx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SVG specific functionality (Masternodes, Darksend, InstantX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s:%u)"), "128.127.106.235", mainParams.GetDefaultPort()));
    ...
    strUsage += HelpMessageOpt("-anonymizesalvageamount=<n>", strprintf(_("Keep N SVG anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The Nitrous Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that SVG is running in a usable environment with all
...
#endif
    // Make sure only a single SVG process is using the data directory.
...
// ********************************************************* Step 8: load wallet
    ...
    nAnonymizeSVGAmount = GetArg("-anonymizesalvageamount", 0);
    if (nAnonymizeSVGAmount > 999999) nAnonymizeSVGAmount = 999999;
    if (nAnonymizeSVGAmount < 2) nAnonymizeSVGAmount = 2;
